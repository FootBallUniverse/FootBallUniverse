using UnityEngine;
using System.Collections;

public class JP_Check_1 : MonoBehaviour
{
    private Player_1_Script m_TeamJP_1;
    private Vector3 m_Check_JP1;
    private Fade_1 m_Fade_JP_1;

    // Use this for initialization
    void Start()
    {

        GameObject m_Team_UI = this.gameObject.transform.parent.gameObject;
        GameObject m_Fade_1 = m_Team_UI.transform.FindChild("Fade_In_Out_1").gameObject;
        m_TeamJP_1 = m_Team_UI.GetComponent<Player_1_Script>();
        m_Fade_JP_1 = m_Fade_1.GetComponent<Fade_1>();
        m_Check_JP1.x = 0.0f;
        m_Check_JP1.y = 2.0f;
        m_Check_JP1.z = -0.6f;
        
    }

    // Update is called once per frame
    void Update()
    {
        if (m_Fade_JP_1.m_FadeFlag == 1 && m_TeamJP_1.m_Country[3].m_Flag == 3)
        {
            m_Check_JP1.y = 0.3f;
        }
        else
        {
            m_Check_JP1.y = 2.0f;
        }

        this.transform.position = m_Check_JP1;
    }
}
