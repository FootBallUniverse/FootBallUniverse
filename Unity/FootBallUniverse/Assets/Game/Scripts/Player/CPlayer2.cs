...
public class CPlayer2 : CPlayer {
	...
	void Start () {
		...
		// カメラをセット
		m_camera = this.gameObject.transform.parent.FindChild("Player2Camera").GetComponent<PlayerCamera>();
	...
	void Update () 
	{
        ...
        CGaugeManager.m_p2Gauge = m_gauge.m_gauge;
	...
	void LateUpdate()
	{
		...
		// プレイヤーのカメラを更新する
		this.transform.parent.FindChild("Player2Camera").GetComponent<PlayerCamera>().CameraUpdate();
	...
	// @Param	none
	// @Return	none
	// @Data    2015/01/11  @Update 2015/01/11  @Author T.Takeuchi
	...
	public override void ChangeViewPoint()
	{
		...
		case CPlayerManager.eVIEW_POINT_STATUS.ePLAYER:
			...
			if (Input.GetKeyDown (InputXBOX360.P2_XBOX_X) || Input.GetKeyDown (InputXBOX360.P2_XBOX_Y) || Input.GetKeyDown (InputXBOX360.P2_XBOX_B) || Input.GetKeyDown (InputXBOX360.P2_XBOX_A))
			{
			...
			// ボールの方向に向ける
			if (Input.GetKey (InputXBOX360.P2_XBOX_B) && m_isBall == false) 
			{
			...
			// 自分のゴールへ向ける
			if (Input.GetKey (InputXBOX360.P2_XBOX_A)) {
    ...
    {
        // 右アナログスティックが押し込まれたら
        if (Input.GetKeyDown(InputXBOX360.P2_XBOX_RIGHT_ANALOG_PRESS))
        {
	...
	public void PlayerTutorial()
	{
		...
		// 移動
		if (this.m_controlePermission.move_x) speed.x = Input.GetAxis(InputXBOX360.P2_XBOX_LEFT_ANALOG_X);
		if (this.m_controlePermission.move_z) speed.z = Input.GetAxis(InputXBOX360.P2_XBOX_LEFT_ANALOG_Y);
		this.Move(speed);

		// 回転
		if (this.m_controlePermission.rotate_x) angle.x = Input.GetAxis(InputXBOX360.P2_XBOX_RIGHT_ANALOG_X);
		if (this.m_controlePermission.rotate_y) angle.y = Input.GetAxis(InputXBOX360.P2_XBOX_RIGHT_ANALOG_Y);
		this.Rotation(angle);
