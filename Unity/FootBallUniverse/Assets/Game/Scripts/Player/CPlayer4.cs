...
public class CPlayer4 : CPlayer {
	...
	void Update () 
	{
        ...
        CGaugeManager.m_p4Gauge = m_gauge.m_gauge;
	...
	void LateUpdate()
	{
		...
		// プレイヤーのカメラを更新する
		this.transform.parent.FindChild("Player4Camera").GetComponent<PlayerCamera>().CameraUpdate();
		
		// 最後に位置をマネージャークラスにセットしておく
		CPlayerManager.m_player4Transform = this.transform;
	...
	private void PlayerStatusNone()
	{
		...
		// 回転
		Vector2 angle = new Vector2(Input.GetAxis(InputXBOX360.P4_XBOX_RIGHT_ANALOG_X), Input.GetAxis(InputXBOX360.P4_XBOX_RIGHT_ANALOG_Y));
		this.Rotation(angle);
	...
	{
		// 回転
		Vector2 angle = new Vector2(Input.GetAxisRaw(InputXBOX360.P4_XBOX_RIGHT_ANALOG_X), Input.GetAxisRaw(InputXBOX360.P4_XBOX_RIGHT_ANALOG_Y));
		this.Rotation(angle);
    ...
    public override void ChangeViewPoint()
    {
        switch (m_viewPointStatus)
        {
            ...
            // 3Pの方向に向ける
            if (Input.GetKey(InputXBOX360.P4_XBOX_X))
            {
                m_trans.LookAt(CPlayerManager.m_player3Transform);
	...
	public void PlayerTutorial()
	{
		...
		// 移動
		if (this.m_controlePermission.move_x) speed.x = Input.GetAxis(InputXBOX360.P4_XBOX_LEFT_ANALOG_X);
		if (this.m_controlePermission.move_z) speed.z = Input.GetAxis(InputXBOX360.P4_XBOX_LEFT_ANALOG_Y);
		this.Move(speed);

		// 回転
		if (this.m_controlePermission.rotate_x) angle.x = Input.GetAxis(InputXBOX360.P4_XBOX_RIGHT_ANALOG_X);
		if (this.m_controlePermission.rotate_y) angle.y = Input.GetAxis(InputXBOX360.P4_XBOX_RIGHT_ANALOG_Y);
		this.Rotation(angle);
