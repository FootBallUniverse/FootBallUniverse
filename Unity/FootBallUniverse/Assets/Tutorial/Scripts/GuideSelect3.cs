...
public class GuideSelect3 : MonoBehaviour
{
    Guide3[] m_Guide = new Guide3[6];
    bool m_Control;
    int m_PosFlag;
    Vector3 m_BasePos;
    ...
    void Update()
    {
        ...
        // LSを右に倒した
        if (Input.GetAxis(InputXBOX360.P3_XBOX_LEFT_ANALOG_X) >= 1.0f && m_Control == false)
        {
            if (m_PosFlag < 5)
            {
                m_PosFlag += 1;
        ...
        // LSを左に倒した
        else if (Input.GetAxis(InputXBOX360.P3_XBOX_LEFT_ANALOG_X) <= -1.0f && m_Control == false)
        {
            if (m_PosFlag > 0)
            {
                m_PosFlag -= 1;
        ...
        else if (Input.GetAxis(InputXBOX360.P3_XBOX_LEFT_ANALOG_X) >= -0.3f && Input.GetAxis(InputXBOX360.P3_XBOX_LEFT_ANALOG_X) <= 0.3f)
        {
            m_Control = false;
        }
        for (int i = 0; i < 6; i++)
